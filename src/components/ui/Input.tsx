...
const Input = forwardRef<HTMLInputElement, InputProps>(
  ({ label, error, fullWidth = false, helperText, className = '', disabled, ...props }, ref) => {
    // Base styles
      ...
      <div className={`mb-4 ${fullWidth ? 'w-full' : ''}`}>
        {label && (
          <label 
            ...
            className={`block text-sm font-medium mb-1 ${disabled ? 'text-gray-500' : 'text-black'}`}
          >
            {label}
          </label>
        )}
        ...
        {helperText && !error && (
          <p className={`mt-1 text-sm ${disabled ? 'text-gray-500' : 'text-[var(--sage-green)]'}`}>{helperText}</p>
        )}
      </div>
    );
  }
