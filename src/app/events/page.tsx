...
export default function EventsPage() {
  ...
  return (
    <div className="min-h-screen">
      {/* Hero Section with Search - Enhanced with overlay and better contrast */}
      <div className="relative bg-gradient-to-r from-[var(--sage-green)] to-[var(--sage)] py-12 px-4 mb-0">
        ...
        <div className="container mx-auto max-w-6xl relative z-10">
          <div className="mb-8 text-center">
            ...
            <p className="text-lg md:text-xl text-white/95 max-w-2xl mx-auto font-light">
              Find and join the best events from around the world
            </p>
          </div>

          {/* Search Bar - Improved with better shadows and focus states */}
            ...
            <form onSubmit={handleSearch} className="relative">
              <div className="flex w-full overflow-hidden rounded-lg shadow-xl">
                <Input
                  ...
                  fullWidth
                  className="rounded-none py-3 pl-12 pr-4 text-md border-0 bg-white/98 focus:ring-2 focus:ring-[var(--sage-green)] focus:bg-white transition-all"
                />
                <Button 
                  type="submit" 
                  className="px-6 rounded-none bg-[var(--sage-green)] hover:bg-[var(--sage-green)]/90 transition-colors" 
                  size="md"
                >
      ...
      <div className="container pl-4 max-w-7xl pb-16">
        ...
        <div className="flex flex-col md:flex-row gap-6">
          {/* Filters Sidebar - Enhanced with better styling */}
          <aside className={`md:w-56 lg:w-64 flex-shrink-0 md:h-fit transition-all duration-300 ease-in-out ${showMobileFilters ? 'max-h-[1000px] opacity-100' : 'max-h-0 opacity-0 overflow-hidden'} md:max-h-none md:opacity-100 md:overflow-visible`}>
              ...
              <Card className="border border-gray-200 shadow-sm overflow-hidden">
                ...
                <CardContent className="p-0">
                  <div className="divide-y divide-gray-200">
                    {/* Categories Section - Better spacing */}
                    <div className="p-4">
                      <button 
                ...
                {countActiveFilters() > 0 && (
                  <CardFooter className="bg-[#f9fafb] border-t border-gray-200 p-3">
                    <Button 
                      ...
                      size="sm" 
                      onClick={resetAllFilters}
                      className="text-xs text-gray-600 border-gray-300 w-full hover:bg-gray-100 transition-colors"
                    >
          ...
          {/* Main Events Content - Enhanced with better spacing */}
          <div className="flex-1">
            {/* Active Filters Display - Improved styling */}
            {countActiveFilters() > 0 && (
                  ...
                  {filters.category && (
                    <div className="bg-[var(--sage-green-light)] text-[var(--sage-green)] rounded-full px-3 py-1 text-sm flex items-center">
                      ...
                      <button 
                        ...
                        className="text-[var(--sage-green)] hover:text-[var(--sage)] rounded-full hover:bg-[var(--sage-green-light)]/80 p-0.5 transition-colors"
            ...
            {/* Events Grid - Enhanced with better card design */}
            {loading ? (
              ...
              <div className="text-center py-16 bg-white rounded-lg shadow-sm border border-gray-200">
                ...
                <Button 
                  ...
                  className="bg-[var(--sage-green)] hover:bg-[var(--sage-green)]/90 transition-colors"
                >
              ...
              <div className="text-center py-16 bg-white rounded-lg shadow-sm border border-gray-200">
                <div className="mx-auto flex items-center justify-center h-12 w-12 rounded-full bg-[var(--sage-green-light)] mb-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {events.map((event) => (
                    <Link href={`/events/${event.slug}`} key={event._id} className="group">
                      <Card className="h-full overflow-hidden border border-gray-200 rounded-lg shadow-sm transition-all duration-300 hover:shadow-md hover:translate-y-[-2px] group-hover:border-[var(--sage-green)]">
                        {event.coverImage ? (
                          <div className="h-48 overflow-hidden relative">
                            ...
                            <div className="absolute top-3 left-3 z-10">
                              <span className="inline-block bg-white/90 backdrop-blur-sm text-[var(--sage-green)] text-xs font-semibold px-2.5 py-1 rounded-full capitalize shadow-sm">
                                {event.category}
                              </span>
                            </div>
                            {/* Price overlay */}
                            <div className="absolute top-3 right-3 z-10">
                              {event.price.isFree ? (
                                <span className="inline-block bg-green-100/90 backdrop-blur-sm text-green-800 text-xs font-semibold px-2.5 py-1 rounded-full shadow-sm">
                                  Free
                                </span>
                              ) : (
                                <span className="inline-block bg-[var(--cognac)]/90 backdrop-blur-sm text-white text-xs font-semibold px-2.5 py-1 rounded-full shadow-sm">
                          ...
                          <div className="h-48 bg-gradient-to-r from-[var(--sage-green)] to-[var(--sage)] flex items-center justify-center relative">
                        ...
                        <CardContent className="p-5">
                          <CardTitle className="font-bold mb-2 text-lg text-gray-900 group-hover:text-[var(--sage-green)] transition-colors line-clamp-1">
                            {event.title}
                          </CardTitle>
                          
                          <CardDescription className="line-clamp-2 mb-4 text-sm text-gray-600">
                          ...
                          <div className="text-xs text-gray-600 space-y-2.5">
                            <div className="flex items-center">
                              <Calendar className="h-3.5 w-3.5 mr-2 text-[var(--sage-green)]" />
                            ...
                            <div className="flex items-center">
                              <Clock className="h-3.5 w-3.5 mr-2 text-[var(--sage-green)]" />
                              <span>
                            ...
                            <div className="flex items-start">
                              {event.location.online ? (
                                <>
                                  <Globe className="h-3.5 w-3.5 mr-2 text-[var(--sage-green)] mt-0.5" />
                                  <span>Online Event</span>
                                </>
                              ) : (
                                <>
                                  <MapPin className="h-3.5 w-3.5 mr-2 text-[var(--sage-green)] mt-0.5" />
                        ...
                        <CardFooter className="px-5 py-3 bg-gray-50 border-t border-gray-200">
                          <div className="w-full flex justify-between items-center">
                            ...
                            <span className="text-[var(--sage-green)] text-xs font-medium group-hover:underline flex items-center transition-all">
                              View Details
                  ...
                  <div className="mt-10 flex justify-center">
                    <div className="inline-flex rounded-md shadow-sm bg-white">
                      <Button
                        ...
                        className="rounded-r-none border-r-0 flex items-center text-sm py-1.5 transition-colors"
                      >
                            ...
                            if (index > 0 && page - array[index - 1] > 1) {
                              return (
                                <React.Fragment key={`ellipsis-${page}`}>
                                  <button className="w-9 h-9 flex items-center justify-center border-t border-b border-gray-300 text-gray-500 cursor-default text-xs">
                                    ...
                                  </button>
                                  <button
                                    key={page}
                                    onClick={() => handleFilterChange('page', page)}
                                    className={`w-9 h-9 flex items-center justify-center border-t border-b border-gray-300 text-sm transition-colors ${
                      ...
                      <div className="sm:hidden border-t border-b border-gray-300 flex items-center px-3">
                      ...
                      <Button
                        ...
                        className="rounded-l-none border-l-0 flex items-center text-sm py-1.5 transition-colors"
                      >
                        Next
